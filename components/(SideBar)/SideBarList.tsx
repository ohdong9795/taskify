    ...
    <>
      <div className="p-4 flex items-center ml-6">
        ...
        <button onClick={handleClick}>
          <Image src={SideBtn} alt="버튼"></Image>
        </button>
      </div>
      <div className="p-4">
        <ul>
      ...
      <div className="absolute bottom-2 left-1 flex justify-between p-4">
        <button className="w-10 h-10" disabled={currentPage === 1} onClick={() => handlePageChange(currentPage - 1)}>
          Previous
        </button>
        <button className="w-10 h-10 ml-10" disabled={!hasNextPage} onClick={() => handlePageChange(currentPage + 1)}>
          Next
        </button>
