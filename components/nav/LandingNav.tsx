  ...
  return (
    <nav className="fixed top-0 left-0 w-full h-[70px] bg-black shrink-0 flex justify-between items-center px-[20px] z-50">
      <div className="flex flex-row">
        ...
        <Taskify className="w-[80px] h-[35px] text-white hidden t:block" />
      </div>
      <div className="flex flex-row t:gap-[36px] gap-[20px]">
        <Link href="/login">
          <div className="t:text-sm text-base text-white ">로그인</div>
        </Link>
        <Link href="/signup">
          <div className="t:text-sm text-base text-white ">회원가입</div>
        </Link>
      </div>
    </nav>
  );
